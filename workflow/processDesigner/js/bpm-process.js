...
PMProject.prototype.getInitProcess = function () {
	...
	if(patternFlag=='FormServerDesigner'){
		...
		processData='<uwws:ProcessContent xmlns:uwws="http://workflow.neusoft.com/model/" xmlns:model="http://workflow.neusoft.com/model/">'
					...
					+'          <ExtendProperty name="cache_ext_radio" value="0"/>'
...
PMProject.prototype.fromXmlToJson=function (processObj,type,nodePoolMap){
	...
	if(type=="activity"){
        ...
        var xmlStrProcess = PMDesigner.xmlToStr(processObj);
        for (var key in PMDesigner.mapForOldIDAndNewID) {
            xmlStrProcess = xmlStrProcess.replaceAll(key,PMDesigner.mapForOldIDAndNewID[key]);
        }
        processObj = PMDesigner.strToXml(xmlStrProcess);
        manualNodes=$(processObj).find("ManualNode");
...
//    		data.error="";
        ...
        $.ajax({
            ...
            data: DESUtils.encode(dirtyObjectXml,"ehalldes"),
            async:false,
